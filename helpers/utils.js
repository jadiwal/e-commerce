...
/**
 * @description
 * the following method creates a jwt token using a payload of user id and email for user and admin id and email
 * @param {object} jwtPayload the jwt payload consisting of user id and email for user and admin id and email
 * @returns the jwt token
 */
export const getJwtToken = (jwtPayload, type) => {
  if (type == "admin") {
    return jwt.sign({
...
/**
 * @description
 * the following method receive's .env file path and returns it in a object format
 * @param {string} filePath the file path for the .env file
 * @returns a JSON Object with the parsed enviroment parameters
 */
export const loadEnv = async (filePath) => {

    return new Promise((resolve, reject) => {
      readFile(filePath, { encoding : 'utf-16le' }, function (err, data) {
        if (err) reject("Env Load Error : " + err)
        var formatted_data = {};
        data.split(/\r?\n/).forEach(function(line,i){
          const __string_line = line.toString();
          const __line_segment = __string_line.toString().split("=");
          if (__line_segment.length > 1) {
            formatted_data[__line_segment[0]] = __line_segment[1];
          }
        })
...
/**
 * @description
 * the following method receive's user's and admin's password from log in request and the password saved in the database
 * and then verifies both of them
 * @param {string} plainTextPassword the password entered by the user and admin during log in
 * @param {string} hashPassword the password extracted from the database
 * @returns a boolean confirming the password verification
 */
